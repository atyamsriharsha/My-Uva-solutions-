...
struct team
{
	...
	int losses ;
	int points ;
	team() :goals_against(0),wins(0),ties(0),losses(0),points(0),goals_scored(0),games_played(0) {}
};
...
struct tournament
{
	...
	int teams_num ;
	vector<team> teams ;
	void add_game(const game_info& game_info) ;
	void update_team_info(const string& name,int goals_scored,int goals_against) ;
};

void tournament::add_game(const game_info& game_info)
{
	update_team_info(game_info.team1,game_info.goals1,game_info.goals2) ;
	update_team_info(game_info.team2,game_info.goals2,game_info.goals1) ;
}
...
int main(int argc, char const *argv[])
{
  ...
  while(n--) 
  {
    ...
    for(int i = 0; i < trn.teams_num; i++) 
    {
      cout << i+1 << ") " << trn.teams[i].name << " " << trn.teams[i].points << "p, " 
      << trn.teams[i].games_played << "g (" << trn.teams[i].wins << "-" << trn.teams[i].ties 
      << "-" << trn.teams[i].losses << "), " << trn.teams[i].goals_scored - trn.teams[i].goals_against 
      << "gd (" << trn.teams[i].goals_scored << "-" << trn.teams[i].goals_against << ")" << endl;
    }
    if (n > 0)
      cout << endl;
  }
